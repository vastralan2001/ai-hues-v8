...
## 五、浏览器截图评估（2026-06-18）

用 Playwright 抓了 9 个页面 × 桌面/移动端共 18 张截图，保存在 `screenshots/eval-2026-06-17/`。

主要观察：

| 页面 | 状态 | 问题/建议 |
|------|------|-----------|
| 首页 | ✅ 良好 | 搜索框 + 分类卡片清晰；Cookie Banner 会遮挡底部少量内容 |
| 工具目录 | ⚠️ 已修复 | 原来大量工具同时显示 NEW badge，意义被稀释；已把 NEW 限制为只有 `kimi-code` |
| 工具详情（Humanize） | ✅ 良好 | 顶部现在正确显示工具名和描述 |
| 定价页 | ⚠️ 待决策 | 视觉干净，但 CTA 没有真实支付链路，上线前需加 "Coming soon" 或隐藏 |
| 隐私政策 | ✅ 良好 | 文案清晰，页脚和 sitemap 已链接 |
| Wishlist | ✅ 良好 | 表单 + 投票列表正常，有邮箱字段可后续对接通知 |
| 博客文章 | ✅ 良好 | 实际文章可正常访问；测试用的错误 slug 已 404（符合预期） |
| 移动端首页/定价 | ⚠️ 可优化 | Cookie Banner 占用底部空间较多，但不阻断主要操作 |

## 六、Newsletter 上线方案

如果你希望 Newsletter 跟着站点一起上线，有四种常见方案：

**A. Resend + 自建 API（推荐）**
- 优点：品牌完全自定义、每天有 3,000 封免费额度、可发 React Email、数据自己掌控。
- 缺点：需要域名 DNS 验证（SPF/DKIM）、要维护一个订阅 API。
- 适合：想长期做、有域名、愿意轻度运维。

**B. Brevo / Mailchimp / Buttondown 等 SaaS**
- 优点：有现成表单/弹窗、自动处理退订/送达率、免费额度足够早期。
- 缺点：页面会跳转到第三方或嵌入 iframe，品牌一致性弱。
- 适合：快速验证、不想自己维护邮件服务。

**C. 先收集到飞书多维表格 / Notion / Airtable**
- 优点：零成本、团队 already 用飞书、可手动导出邮件列表。
- 缺点：不能自动发邮件，只是“收集意向”。
- 适合：还没确定内容策略、想先囤一批订阅者。

**D. 先隐藏 Newsletter，上线后再补**
- 优点：最简单、无合规风险。
- 缺点：少一个用户留存渠道。
- 适合：MVP 上线、不想分散精力。

**我的建议**：短期选 **A（Resend）** 或 **D（先隐藏）**。如果域名和 Resend 账号能当天搞定，A 的性价比最高；否则 D 最稳妥，避免让用户留下邮箱却收不到邮件。

## 七、明天验收建议

1. 打开 `docs/ai-writing-evaluation-2026-06-17-v2.md`，在每工具下方的“人工评估”栏填上你的判断。
2. 查看 `screenshots/eval-2026-06-17/` 里的截图，确认 UI 是否符合预期。
3. 访问 `/privacy` 和任意工具详情页，确认页脚、Cookie Banner、工具标题都正常。
4. 决定：保留 Pricing 页面并加 "Coming soon"、隐藏 Pricing、还是接入真实支付。
5. 决定：Newsletter 选 A/B/C/D 哪条路。
