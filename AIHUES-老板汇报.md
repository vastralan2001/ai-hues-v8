...
### 决策 2：Wishlist 需求投票后端排期

**现状**：`/wishlist` 页面纯前端占位，7 条假数据。提交按钮无实际逻辑，投票刷新即丢失。

**需要后端开发**：
- `wishes` 表（需求内容、投票数、状态）
- POST / GET API（提交需求、获取列表）
- 投票接口（支持/反对）
- 用户身份（至少匿名 ID，防刷票）

**建议**：如果近期要上，需后端同学排期；如果优先级不高，可继续前端占位。
...
2. **若决策 2 排期**：与后端同学对接 Wishlist 表结构 + API 设计
3. **若决策 3 通过**：制定 HTML→React 迁移计划，分批次执行
