...
| 9 | **首页统计数字修复** — 57/30/8/19/3 全部正确 | ✅ 刚推送 |
...
### 决策 3：26 个静态 HTML 工具迁移

**现状**：57 个工具中，**26 个仍是静态 HTML 页面**（跳转到 `.html`），与 31 个 React 工具的交互体验、导航一致性差距明显。

**两种方案**：
| 方案 | 工作量 | 效果 |
|------|--------|------|
| A. 逐步迁移为 React 组件 | 中（每个 2–4h）| 体验完全统一，可复用现有组件库 |
| B. 至少统一导航和布局 | 低（改 HTML 模板）| 视觉上统一，但交互仍有差距 |

**建议**：选方案 A，分批迁移（每次 5–10 个），不阻塞主流程。
