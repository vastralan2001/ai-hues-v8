...
(function () {
  ...
  function saveAchievements(data) {
    try {
      localStorage.setItem(LS_ACHIEVEMENTS, JSON.stringify(data));
    } catch (e) {}
  }
  ...
  function saveToolUsage(data) {
    try {
      localStorage.setItem(LS_TOOL_USAGE, JSON.stringify(data));
    } catch (e) {}
  }
  ...
  function saveVisits(arr) {
    try {
      localStorage.setItem(LS_VISITS, JSON.stringify(arr));
    } catch (e) {}
  }
  ...
  function saveGamesPlayed(data) {
    try {
      localStorage.setItem(LS_GAMES_PLAYED, JSON.stringify(data));
    } catch (e) {}
  }
  ...
  function trackToolUsage(toolUrl, category) {
    ...
    var todayTools = Object.keys(usage).filter(function (k) {
      ...
      return usage[k].some(function (ts) {
  ...
  function getAchievements() {
    ...
    ACHIEVEMENT_DEFS.forEach(function (def) {
      ...
      result[def.id] = {
        ...
        progress:
          def.type === 'progress'
            ? Math.min(currentProgress, def.target)
            : ach.unlocked
              ? 1
              : 0,
  ...
  function getUnlockedCount() {
    ...
    return Object.keys(ach).filter(function (k) {
      return ach[k] && ach[k].unlocked;
    }).length;
  }
  ...
  function renderAchievementsPanel(container) {
    var el =
      typeof container === 'string'
        ...
        : container;
    if (!el) {
      console.warn('[Achievements] container not found:', container);
      return;
    }
    el.innerHTML = '';
    ...
    var unlockedCount = ids.filter(function (id) {
      return data[id].unlocked;
    }).length;
    ...
    ACHIEVEMENT_DEFS.forEach(function (def, i) {
      ...
      var progressPct =
        def.type === 'progress' && def.target > 0
      ...
      card.innerHTML = [
        '<div class="ach-badge" style="width:50px;height:50px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:24px;flex-shrink:0;background:' +
          (isUnlocked ? 'linear-gradient(135deg,#f3e8ff,#ede9fe)' : '#f8f9fb') +
          ';">' +
          def.icon +
          '</div>',
        ...
        isUnlocked && ach.unlockedAt
          ...
          : '',
        '</div>',
        '<div style="font-size:20px;flex-shrink:0;color:' +
          statusColor +
          ';">' +
          statusIcon +
          '</div>',
  ...
  /* ============================================================
     ...
     ============================================================ */
  function autoInit() {
    // Only auto-init on tool and game pages.
  ...
  var API = {
    ...
    // Expose definitions for external use.
